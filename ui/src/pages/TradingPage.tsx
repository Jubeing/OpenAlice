...
function DynamicBrokerFields({ fields, values, showSecrets, onChange, brokerType }: {
  ...
  const handleRefreshToken = async () => {
    const appKey = String(values['appKey'] ?? '').trim()
    const appSecret = String(values['appSecret'] ?? '').trim()
    const currentToken = String(values['refreshToken'] ?? '').trim()
    if (!appKey || !appSecret) { setRefreshError('Enter APP_KEY and APP_SECRET first.'); return }
    if (!currentToken) { setRefreshError('Enter REFRESH_TOKEN first.'); return }
    setRefreshing(true); setRefreshError('')
    try {
      const result = await (api.trading as any).refreshLongbridgeToken(appKey, appSecret, currentToken)
      if (result.error) {
        // OAuth2 invalid_grant means the token is not a valid refresh token
        // (e.g., was obtained via a different OAuth client). User must re-authenticate.
        if (result.error.includes('invalid_grant') || result.error.includes('403208')) {
          setRefreshError('Token is invalid or revoked. Please click "Re-authenticate" to get a new token via browser.')
        } else {
  ...
  return (
    <div className="space-y-3">
      {fields.map((f) => {
        const isLbToken = brokerType?.type === 'longbridge' && f.name === 'refreshToken'
        ...
        if (isLbToken) {
          return (
            <div key={f.name}>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    title="Exchange refresh token for new access token"
                  >
              ...
              {refreshError && <p className="text-[11px] text-red mt-1">{refreshError}</p>}
