...
const LOGIN_METHODS: { value: LoginMethod; labelKey: string; subtitleKey: string; hintKey: string }[] = [
  { value: 'claudeai', labelKey: 'claudePro', subtitleKey: 'claudeProSubtitle', hintKey: 'claudeProHint' },
  { value: 'api-key', labelKey: 'apiKey', subtitleKey: 'claudeProSubtitle', hintKey: 'apiKeyHint' },
]
...
export function AIProviderPage() {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      <PageHeader title={t.aiProvider.title} description={t.aiProvider.titleDesc} />

      {config ? (
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6">
          <div className="max-w-[880px] mx-auto">
            {/* Backend */}
            <ConfigSection title={t.aiProvider.backend} description={t.aiProvider.backendDesc}>
              <div className="grid grid-cols-3 gap-3">
                <BackendCard
                  ...
                  title="Claude"
                  description={t.aiProvider.claudeDesc}
                />
                <BackendCard
                  ...
                  title="OpenAI"
                  description={t.aiProvider.openaiDesc}
                />
                <BackendCard
                  ...
                  title="Vercel AI SDK"
                  description={t.aiProvider.vercelAiSdkDesc}
                />
              </div>
            </ConfigSection>
            ...
            {uiBackend === 'agent-sdk' && (
              <ConfigSection title={t.aiProvider.authentication} description={t.aiProvider.authDesc}>
            ...
            {uiBackend === 'openai' && (
              <ConfigSection title={t.aiProvider.model} description={t.aiProvider.modelDesc}>
            ...
            {uiBackend === 'vercel-ai-sdk' && (
              <ConfigSection title={t.aiProvider.model} description={t.aiProvider.modelFullDesc}>
...
function OpenAIForm({ aiProvider }: { aiProvider: AIProviderConfig }) {
  ...
  return (
    <>
      <Field label={t.aiProvider.modelId}>
        <select
        ...
        >
          ...
          <option value="__custom__">{t.aiProvider.customModelId}</option>
        </select>
      </Field>

      {!model && (
        <Field label={t.aiProvider.customModelId}>
          <input
            className={inputClass}
            value={customModel}
      ...
      <Field label={t.aiProvider.apiKey}>
        <div className="relative">
          <input
            ...
            placeholder={hasKey ? t.aiProvider.configured : 'sk-...'}
          />
          {hasKey && (
            <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-green">{t.aiProvider.active}</span>
          )}
        </div>
...
function ModelForm({ aiProvider }: { aiProvider: AIProviderConfig }) {
  ...
  const handleProviderChange = (newUiProvider: string) => {
    ...
    if (newUiProvider === 'custom') {
      ...
      setBaseUrl(PROVIDER_BASE_URLS[newUiProvider] || '')
  ...
  return (
    <>
      ...
      {isCustomMode && (
        <Field label={t.aiProvider.apiFormat}>
          <select
            className={inputClass}
            value={sdkProvider}
      ...
      {(isCustomMode || isCustomModelInStandard || (!isCustomMode && model === '')) && (
        <Field label={isCustomMode ? t.aiProvider.modelId : t.aiProvider.customModelId}>
          <input
            className={inputClass}
      ...
      <div className="grid grid-cols-2 gap-3">
        {LOGIN_METHODS.map((m) => (
          <BackendCard
            ...
            title={t.aiProvider.loginMethod[m.labelKey as keyof typeof t.aiProvider.loginMethod] as string}
            description={t.aiProvider.loginMethod[m.subtitleKey as keyof typeof t.aiProvider.loginMethod] as string}
          />
        ))}
      </div>
      <p className="text-[11px] text-text-muted mt-2">
        {t.aiProvider.loginMethod[LOGIN_METHODS.find((m) => m.value === loginMethod)?.hintKey as keyof typeof t.aiProvider.loginMethod] as string}
      </p>

      {loginMethod === 'api-key' && (
        <Field label={t.aiProvider.apiKey + ' (Anthropic)'}>
          <div className="relative">
            <input
              className={inputClass}
              type="password"
              value={apiKey}
              onChange={(e) => setApiKey(e.target.value)}
              placeholder={aiProvider.apiKeys?.anthropic ? t.aiProvider.configured : 'sk-ant-...'}
            />
