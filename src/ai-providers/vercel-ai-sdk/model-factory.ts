...
export async function createModelFromConfig(override?: ModelOverride): Promise<ModelFromConfig> {
  ...
  switch (p) {
    case 'anthropic':
    case 'minimax': {
      // minimax uses Anthropic-compatible API via @ai-sdk/anthropic
      // Base URL defaults to https://api.minimaxi.com/anthropic/v1/messages, but can be overridden via config.baseUrl
      const { createAnthropic } = await import('@ai-sdk/anthropic')
      const minimaxUrl = url || 'https://api.minimaxi.com/anthropic/v1/messages'
      const client = createAnthropic({
        apiKey: resolveApiKey('anthropic'), // ANTHROPIC_API_KEY
        baseURL: minimaxUrl,
      })
      return { model: client(m), key: `${p}:${m}:${minimaxUrl}` }
    }
    case 'openai': {
