...
export const longbridgeConfigFields: BrokerConfigField[] = [
  { name: 'appKey', type: 'text', label: 'APP_KEY', required: true, sensitive: true, description: 'Longbridge App Key from developer portal.' },
  { name: 'appSecret', type: 'password', label: 'APP_SECRET', required: true, sensitive: true, description: 'Longbridge App Secret.' },
  { name: 'accessToken', type: 'password', label: 'ACCESS_TOKEN', required: true, sensitive: true, description: 'Access Token. Auto-refreshes on the 1st of each month.' },
...
export class LongbridgeBroker implements IBroker {
  ...
  constructor(config: {
    ...
    autoRefresh?: boolean; tokenExpiry?: string; lastTokenRefresh?: string; paper?: boolean
  }) {
    this.id = config.id
  ...
  private async ensureValidToken(): Promise<void> {
    ...
    const lastRefresh = this.config.lastTokenRefresh ? new Date(this.config.lastTokenRefresh).getTime() : 0
    const daysSinceRefresh = (now.getTime() - lastRefresh) / (1000 * 60 * 60 * 24)

    // Auto-refresh: if token expiring within 7 days OR (first 5 days of month AND last refresh > 25 days ago)
    const needsExpiringRefresh = this.config.tokenExpiry && isTokenExpiringSoon(this.config.tokenExpiry, 7)
    const needsMonthlyRefresh = dayOfMonth <= 5 && daysSinceRefresh > 25

    if (!needsExpiringRefresh && !needsMonthlyRefresh) return
    ...
    this.config.lastTokenRefresh = now.toISOString()
  ...
  async refreshToken(): Promise<{ token: string; expiredAt: string }> {
    ...
    this.config.accessToken = result.token
    this.config.tokenExpiry = result.expiredAt
    this.config.lastTokenRefresh = new Date().toISOString()
    return result
  }
  ...
  async getAccount(): Promise<AccountInfo> {
    try {
      ...
      for (const b of balances) {
        ...
        const net = Number(b.netAssets?.toString?.() ?? 0)
        const tot = Number(b.totalCash?.toString?.() ?? 0)
        const bp = Number(b.buyPower?.toString?.() ?? 0)
        if (cur === 'USD' || cur === 'usd') {
  ...
  async getPositions(): Promise<Position[]> {
    try {
      ...
      return raw.map((p: any) => {
        const qty = Number(p.quantity?.toString?.() ?? 0)
        const cost = Number(p.costPrice?.toString?.() ?? 0)
        ...
        return {
          contract: { ...makeContract(p.symbol), description: p.symbolName ?? '', currency },
